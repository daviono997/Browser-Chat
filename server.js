...
// ── Stream WebSocket (/api/stream) — binary audio/video relay ─────────────────
// Two clients join the same room (sorted uid pair) and exchange raw binary chunks.
const streamWss   = new WebSocket.Server({ noServer: true });
const streamRooms = new Map(); // roomId → Set<ws>

streamWss.on('connection', (ws, req) => {
  const params = new URLSearchParams(urlModule.parse(req.url).query || '');
  const roomId = params.get('room');
  if (!roomId) { ws.close(); return; }

  if (!streamRooms.has(roomId)) streamRooms.set(roomId, new Set());
  ...
  ws.on('close', () => {
    room.delete(ws);
    if (room.size === 0) streamRooms.delete(roomId);
  });
  ws.on('error', () => { try { ws.close(); } catch (_) {} });
});
...
// ── POST /api/register — polling clients announce their uid on startup ─────────
// This lets the server know to queue signals/DMs for this uid
const pollingClients = new Set(); // uids currently connected via polling
app.post('/api/register', (req, res) => {
  const { uid } = req.body || {};
  if (uid) {
    pollingClients.add(uid);
    // Flush any already-queued messages back in the response
    const queued = pending.has(uid) ? pending.get(uid).splice(0) : [];
    if (pending.has(uid) && pending.get(uid).length === 0) pending.delete(uid);
...
// ── Frame endpoints — latest-frame-only store for video calls ─────────────────
// Each room stores only the single most recent frame. Receiver polls at 30fps.
// This bypasses the signal queue entirely so video is independent of chat lag.
const frameStore = new Map(); // roomId → {b64, ts}

app.post('/api/frame/:room', (req, res) => {
  let body = '';
  req.on('data', c => body += c);
  req.on('end', () => {
    if(body) frameStore.set(req.params.room, { b64: body, ts: Date.now() });
    res.end('ok');
  });
});

app.get('/api/frame/:room', (req, res) => {
  const entry = frameStore.get(req.params.room);
  if(!entry || Date.now()-entry.ts > 2000) { // stale after 2s
